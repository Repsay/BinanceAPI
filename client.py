...
class Client(object):
    ...
    def get_best_symbols(self, n, quote="BTC"):
    ...
    def get_all_asset_balance(self, n=0):
    ...
    def updateCandle(self, symbol, interval=KLINE_INTERVAL_1MINUTE):
            
            #check excistence of the folders and create structure if not already there
            filename = f"{self.MAIN_PATH}/data/candles/{interval}"
            lastTime = 0
            ...
            for candle in self.get_historical_candles_generator(symbol, lastTime, interval):
    ...
    def updateAllCandles(self, symbols, interval=KLINE_INTERVAL_15MINUTE):
        
        symbols = [symbol for symbol in symbols if "BTC" in symbol]

        for symbol in symbols:
            self.updateCandle(symbol, interval)
            print(f"{symbol} Done")

    def updateEMA(self, symbol, emas, interval=KLINE_INTERVAL_15MINUTE):

        First = True

        #check files and directories
        filename = f"{self.MAIN_PATH}/data/candles/{interval}"
        filename2 = f"{self.MAIN_PATH}/data/ema/{interval}"
    ...
    def updateAllEMA(self, symbols, emas, interval=KLINE_INTERVAL_15MINUTE):
        ...
        for i in range(length):
            ...
            self.updateEMA(symbols[i], emas, interval)

    def updateMA(self, symbol, mas, interval, ma_type=MA_Type.SMA):
        ...
        #check files and directories
        filename = f"{self.MAIN_PATH}/data/candles/{interval}"
        filename2 = f"{self.MAIN_PATH}/data/{folder}/{interval}"
    ...
    def updateAllMA(self, symbols, mas, interval, ma_type):
        ...
        for i in range(length):
            ...
            if cur_amount < new_amount:
                cur_amount = new_amount
                print("*"*cur_amount, end="\r")
            self.updateMA(symbols[i], mas, interval, ma_type)


    def get_all_intervals(self):
        intervals = [self.KLINE_INTERVAL_1MINUTE, self.KLINE_INTERVAL_3MINUTE, self.KLINE_INTERVAL_5MINUTE, self.KLINE_INTERVAL_15MINUTE, self.KLINE_INTERVAL_30MINUTE, self.KLINE_INTERVAL_1HOUR, self.KLINE_INTERVAL_2HOUR, self.KLINE_INTERVAL_4HOUR, self.KLINE_INTERVAL_6HOUR, self.KLINE_INTERVAL_8HOUR, self.KLINE_INTERVAL_12HOUR, self.KLINE_INTERVAL_1DAY, self.KLINE_INTERVAL_3DAY, self.KLINE_INTERVAL_1WEEK]
